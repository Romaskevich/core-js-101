...
function isInsideCircle(circle, point) {
  const toPoint = Math.sqrt((point.x - circle.center.x) ** 2 + (point.y - circle.center.y) ** 2);
  return toPoint < circle.radius;
}
...
function findFirstSingleChar(str) {
  return str.split('').filter((el, _, arr) => arr.indexOf(el) === arr.lastIndexOf(el))[0];
}
...
function evaluateTicTacToePosition(position) {
  ...
  for (let j = 0; j < position.length; j += 1) {
    ...
    if (position[j][0]) {
      if (position[j][0] === position[j][1] && position[j][0] === position[j][2]) {
        return position[j][0];
      }
    }
  }
